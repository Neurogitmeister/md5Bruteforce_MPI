...
#ifndef BENCHMARK
    int first_rank_of_perms_range = 0; // первый    ранк из диапазона, начальные и конечные слова которого требуется отобразить,      задаётся аргументом main
    int last_rank_of_perms_range = 0;  // последний ранк из диапазона, начальные и конечные слова которого требуется отобразить,      задаётся аргументом main
    FILE *fd = NULL;                   // файловый дескриптор, открывающийся на процессах из заданного диапазона для записи начальных и конечных слов
#endif
