...
#ifndef BENCHMARK
    int first_rank_of_perms_range; // первый    ранк из диапазона, варианты перестановок которого требуется отобразить,      задаётся аргументом main
    int last_rank_of_perms_range;  // последний ранк из диапазона, варианты перестановок которого требуется отобразить,      задаётся аргументом main
    FILE *fd = NULL;
#endif
